...
class JWTManager:
    ...
    Built once at startup with the signing key and token lifetime.
    """
    ...
    def create_token(self, claims: dict[str, Any]) -> str:
        ...
        return jwt.encode(to_encode, self._secret_key, algorithm=_ALGORITHM)
    ...
    def decode_token(self, token: str) -> dict[str, Any]:
        ...
        try:
            payload: dict[str, Any] = jwt.decode(
                token, self._secret_key, algorithms=[_ALGORITHM],
            )
        except JWTError as exc:
            raise ValueError(f"Invalid token: {exc}") from exc
        return payload

    async def resolve_user(self, token: str, dao: UserDAO) -> User:
